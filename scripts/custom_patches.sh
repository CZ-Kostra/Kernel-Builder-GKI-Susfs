...
if [[ "${SU_VARIANT}" != "KernelSU-Next" ]]; then
    ...
    if find . -name "*.rej" | grep -q "."; then
        ...
        case "${SU_VARIANT}" in
            "SukiSU-Ultra")
                echo ">>> Initiating SukiSU-Ultra fixup routine..."
...
EOF
            ...
            *)
                echo "[-] Unknown variant ${SU_VARIANT} with rejections. Aborting."
                exit 1
                ;;
        esac
        ...
        echo ">>> ${SU_VARIANT} dynamic fixups applied successfully!"
    else
        echo ">>> Patch applied cleanly to ${SU_VARIANT} with zero rejections!"
    fi
    
    cd ../..
else
    echo ">>> Target is KernelSU-Next. Native 2.20 support detected, skipping custom patches..."
fi
