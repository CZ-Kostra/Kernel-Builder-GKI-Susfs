#!/bin/bash
# scripts/patchfix_SukiSU-Ultra.sh
...
sed -i -E 's/static[[:space:]]+int[[:space:]]+security_context_to_sid_with_policy/int security_context_to_sid_with_policy/g' ../common/security/selinux/ss/services.c
sed -i -E 's/static[[:space:]]+int[[:space:]]+security_sid_to_context_with_policy/int security_sid_to_context_with_policy/g' ../common/security/selinux/ss/services.c
sed -i -E 's/static[[:space:]]+int[[:space:]]+security_compute_av_user_with_policy/int security_compute_av_user_with_policy/g' ../common/security/selinux/ss/services.c

echo ">>> Applying Universal SuSFS 2.20 Patch to SukiSU-Ultra..."
patch -p1 --no-backup-if-mismatch < ../../patches/2.20_universal_susfs.patch || true

# 1. Silently delete KNOWN patch rejections that our awk scripts are about to fix
...
if find . -name "*.rej" | grep -q "."; then
    ...
    # Clean up the unexpected files so the compiler can still attempt a run
    find . -name "*.rej" -type f -delete
else
    echo ">>> No unexpected rejections detected."
fi

echo ">>> Initiating SukiSU-Ultra dynamic fixup routine..."
...
# 10. kernel/feature/selinux_hide.c (Fix 6.6 linkage by removing erroneous 'static' declarations)
...
# 16. kernel/kpm/super_access.c (Fix 6.12 cb_mutex removal in netlink_kernel_cfg)
