...
# Silence Clang -Wpointer-bool-conversion errors for 6.6 extern functions
sed -i 's/if (security_dump_masked_av_fn)/if (\&security_dump_masked_av_fn)/g' kernel/feature/selinux_hide.c
sed -i 's/if (context_struct_compute_av_fn)/if (\&context_struct_compute_av_fn)/g' kernel/feature/selinux_hide.c
